#!/usr/bin/env python3
"""
Gerador Dashboard Lançamento Pago v3
"""
...
def build_daily(p):
    ...
    out={k:[] for k in ["days","spend","impressions","link_clicks","page_view",
                         "init_checkout","purchase","ctr","connect_rate","tx_ic","tx_checkout","tx_conv","cpa","cpm"]}
...
def build_table_rows(agg, col):
    rows=[]
    for _,r in agg.sort_values("purchase",ascending=False).iterrows():
        sp=float(r["spend"]); imp=float(r["impressions"]); lc=float(r["link_clicks"])
        pv=float(r["page_view"]); ic=float(r["init_checkout"]); pur=float(r["purchase"])
        rows.append({
            "n":str(r[col]),"spend":round(sp,2),"imp":int(imp),"lc":int(lc),
            "pv":int(pv),"ic":int(ic),"pur":int(pur),
            "ctr":round(lc/imp*100,2) if imp>0 else None,
...
def meta_tables(df, img_dir):
    ...
    def make(p, col): return build_table_rows(agg_group(p,col), col)
...
def meta_breakdowns(df):
    ...
    def seg_rows(agg, dim):
        ...
        return [{"n":str(r[dim]),"spend":round(float(r["spend"]),2),
                 "pur":int(r["purchase"]),"cpa":safe(r["cpa"])} for _,r in agg.iterrows()]
    ...
    try:
        ...
        agg_age=df_ga[df_ga["age"].isin(AGE_ORDER)].groupby("age").agg(
            spend=("spend","sum"),purchase=("purchase","sum")).reset_index()
        agg_age["_o"]=agg_age["age"].apply(lambda x:AGE_ORDER.index(x) if x in AGE_ORDER else 99)
        agg_age=agg_age.sort_values("_o")
        age_data=seg_rows(agg_age,"age")

        agg_gen=df_ga[df_ga["gender"].isin(["female","male"])].groupby("gender").agg(
            spend=("spend","sum"),purchase=("purchase","sum")).reset_index()
        agg_gen=agg_gen.sort_values("purchase",ascending=False)
        gender_data=seg_rows(agg_gen,"gender")
    except Exception as e:
        print(f"  Aviso GA: {e}"); age_data=[]; gender_data=[]
    ...
    try:
        ...
        agg_pt=df_pt.groupby("platform").agg(
            spend=("spend","sum"),purchase=("purchase","sum")).reset_index()
        agg_pt=agg_pt.sort_values("purchase",ascending=False).head(10)
        platform_data=seg_rows(agg_pt,"platform")
    except Exception as e:
        print(f"  Aviso PT: {e}"); platform_data=[]

    return {"age":age_data,"gender":gender_data,"platform":platform_data}
...
def pesquisa_process(df):
    ...
    for p in PERGUNTAS:
        ...
        graficos.append({"pergunta":p,"opcoes":[
            {"label":str(k),"qtd":int(v),"pct":round(v/total*100,1)} for k,v in vc.items()]})
    ...
    for _,r in df.iterrows():
        row={}
        for p in PERGUNTAS:
            row[p]=str(r[p]) if p in df.columns and pd.notna(r.get(p)) else None
        for col in UTM_COLS:
            row[col]=str(r[col]) if col in df.columns and pd.notna(r.get(col)) else None
        rows.append(row)
...
def inject_all(tpl, meta_k, meta_d, meta_t, meta_bd, hot_k, hot_d, pes):
    ...
    for k,v in [("LANCAMENTO_COD",f"'{LANCAMENTO_COD}'"),("NOME_CLIENTE",f"'{NOME_CLIENTE}'"),
        ...
        html=re.sub(rf"const {k}\s*=\s*[\d\.'""][^;]*",f"const {k}={v}",html,count=1)
...
def main():
    ...
    print("\n[HOTMART]")
    df_hot=load_hotmart()
    h_k,h_d=hotmart_process(df_hot)
    print(f"  ✓ {h_k['qtd']} vendas | R$ {h_k['total']:,.2f}")
    ...
    html=inject_all(TEMPLATE_FILE,m_k,m_d,m_t,m_bd,h_k,h_d,pes)
    ...
    data_json={
        ...
        "kpis":{"spend":m_k["lct"].get("spend"),"vendas":h_k.get("qtd"),
                "faturamento":h_k.get("total"),"cpa":m_k["lct"].get("cpa")}
    }
